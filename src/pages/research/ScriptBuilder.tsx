...
const SCREEN_POP_QUERY = '?lead=--A--lead_id--B--&phone=--A--phone_number--B--&agent=--A--user--B--&campaign=--A--campaign--B--';
...
export default function ScriptBuilder() {
  ...
  const handleWizardSave = async (wizardData: WizardData, isDraft: boolean) => {
    await createScript({
      ...
      ai_prompt: wizardData.aiPrompt || undefined,
      ai_model: wizardData.aiModel || undefined,
      ai_temperature: wizardData.aiTemperature ?? undefined,
    } as any);
    setWizardMode(false);
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-80 p-4 space-y-3" align="end">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-xs text-muted-foreground">In the recording POST send the call's lead_id as <code className="font-mono">leadId</code> and its recording_id as <code className="font-mono">recordingId</code>.</p>
                              </div>
                              <div className="space-y-1">
