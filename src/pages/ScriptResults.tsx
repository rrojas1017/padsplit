...
import { Download, BarChart3, ArrowLeft, LayoutDashboard } from "lucide-react";
...
const generateDynamicReport = lazy(() => import("@/utils/generateDynamicReport").then(m => ({ default: m.generateDynamicReport as any })));
...
export default function ScriptResults() {
  ...
  return (
    <DashboardLayout
      title={script?.name || "Script Results"}
      subtitle={`${uniqueSessions} responses · ${questions.length} questions`}
      actions={
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate("/research/scripts")}>
            <ArrowLeft className="w-4 h-4 mr-2" /> Back
          </Button>
          <Button onClick={handleDownloadReport} disabled={responses.length === 0}>
            <Download className="w-4 h-4 mr-2" /> Executive Report (.docx)
          </Button>
        </div>
      }
    >
