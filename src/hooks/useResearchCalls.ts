...
export function useResearchCalls() {
  ...
  const submitCall = async (submission: CallSubmission) => {
    ...
    // Step 2: persist durable raw_script_answers server-side.
    // The edge function links/creates the research booking, creates the
    // transcription row and writes script_responses. Always invoke, even
    // when raw_script_answers is empty, so the call is linked for analysis.
    try {
      ...
      const { error: persistError } = await supabase.functions.invoke('persist-research-raw-answers', {
        body: {
      ...
      if (persistError) {
        console.error('persist-research-raw-answers returned an error:', persistError);
        toast.warning('Call saved, but it could not be linked for AI analysis');
      }
    } catch (rawErr) {
