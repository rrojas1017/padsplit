...
interface ProbingFollowUpsProps {
  ...
  probeNotes?: Record<number, string>;
  onProbeNoteChange?: (probeIndex: number, note: string) => void;
}

export function ProbingFollowUps({ probes, label = 'Probing follow-ups', variant = 'default', probeNotes, onProbeNoteChange }: ProbingFollowUpsProps) {
        ...
        <div className="space-y-1.5 pt-1">
          {probes.map((probe, idx) => (
            <div key={idx} className="space-y-1">
              ...
              {activated.has(idx) && (
                <Textarea
                  placeholder="Capture member's response..."
                  className="ml-6 text-sm min-h-[60px] bg-background"
                  value={probeNotes?.[idx] || ''}
                  onChange={(e) => onProbeNoteChange?.(idx, e.target.value)}
                />
              )}
            </div>
          ))}
