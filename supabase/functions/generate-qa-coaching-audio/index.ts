...
Generate ONLY the spoken coaching script. No formatting, no quotes around the script, no stage directions.`;
    ...
    if (!elevenLabsResponse.ok) {
      ...
      if (status === 'payment_issue' || elevenLabsResponse.status === 402) {
        ...
        if (!existing || existing.length === 0) {
          await supabase.from('admin_notifications').insert({
            ...
            message: `QA coaching audio generation is failing due to a billing problem: ${message}`,
            severity: 'critical',
      ...
      if (status === 'quota_exceeded' || message.toLowerCase().includes('quota')) {
        ...
        if (!existing || existing.length === 0) {
          await supabase.from('admin_notifications').insert({
            ...
            message: `QA coaching audio quota has been exceeded: ${message}`,
            severity: 'warning',
      ...
      throw new Error(`ElevenLabs TTS error: ${elevenLabsResponse.status} - ${message}`);
    }
