-- Update trigger to only validate contacts on INSERT or when contact fields change
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND NEW.import_batch_id IS NULL THEN
    -- Only check if contact_email was changed and is now empty
    ...
    -- Only check if contact_phone was changed and is now empty
